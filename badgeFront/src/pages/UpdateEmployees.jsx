...
const UpdateEmployee = () => {
    ...
    useEffect(() => {
        const getEmployee = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${url}/employees/${id}/`, {headers, withCredentials: true});
                if (response.data.success) {
                    const employee = response.data.data;
                    setFormData({
                        old_matricule: employee.old_matricule || '',
                        new_matricule: employee.new_matricule || '',
                        ssn: employee.ssn || '',
                    ...
                    if (employee.photo_url) {
                        setOriginalPhoto(employee.photo_url);
    ...
    }, [id, url, navigate, setLoading, getAuthHeaders]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`${url}/employees/${id}/`, data, {headers: {...headers, 'Content-Type': 'multipart/form-data'}, withCredentials: true, timeout: 20000});
            ...
            <div className="max-w-4xl mx-auto">
                <div className="mb-8 flex items-center justify-between">
                    ...
                    <h1 className="text-3xl font-extrabold text-text-primary">
                        <span className="bg-gradient-to-r from-secondary to-secondary-dull bg-clip-text text-transparent">Modifier l'employé</span>
                    </h1>
                </div>
                {loading && !formData.new_matricule ? (
                            ...
                            <div className="bg-gradient-to-r from-primary/5 to-primary-dull/5 p-6 border-b border-gray-100">
                                <div className="flex items-center space-x-6">
                                    ...
                                    <div>
                                        ...
                                        {originalPhoto && !formData.photo && ( <p className="text-sm text-text-secondary mt-1">Photo actuelle conservée</p>)}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center justify-end space-x-4">
                            ...
                            <button type="submit" disabled={loading} className="group relative px-8 py-3 bg-gradient-to-r from-primary to-primary-dull text-white font-semibold rounded-xl shadow-lg shadow-primary/25 cursor-pointer hover:shadow-xl transform transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center space-x-2">
                                ...
                                {loading ? 
                                    ...
                                    <i className="fa-solid fa-pen-to-square transform group-hover:scale-110 transition-transform duration-300"></i>
                                }
                            </button>
                        </div>
                    </form>
